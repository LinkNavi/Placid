...
bool EditorApp::Initialize(int width, int height, const char *title) {
  ...
  if (mapEditor->GetMap().brushes.empty()) {
    PCD::Brush floor =
        mapEditor->CreateBox(PCD::Vec3(-20, -1, -20), PCD::Vec3(20, 0, 20));
    floor.name = "Floor";
...
void EditorApp::UpdateCamera(float dt) {
  cameraPosition.x =
      cameraFocusPoint.x + cameraDistance * sin(cameraYaw) * cos(cameraPitch);
  cameraPosition.y = cameraFocusPoint.y + cameraDistance * sin(cameraPitch);
  cameraPosition.z =
      cameraFocusPoint.z + cameraDistance * cos(cameraYaw) * cos(cameraPitch);
}
...
void EditorApp::Render() {
  ...
  renderer->RenderEntities(
      mapEditor->GetMap().entities, mapEditor->GetSelectedEntityIndex(),
      mapEditor->GetSettings().showEntityIcons, view, proj);

  mapEditor->RenderGizmo(renderer, view, proj);

  if (mapEditor->IsCreating()) {
    renderer->RenderCreationPreview(
        mapEditor->GetCreateStart(), mapEditor->GetCreateEnd(),
...
void EditorApp::GetEditorViewMatrix(float *mat) {
  ...
  mat[4] = r.y;
  mat[8] = r.z;
  mat[12] = -r.x * eye.x - r.y * eye.y - r.z * eye.z;
  mat[1] = u.x;
  mat[5] = u.y;
  mat[9] = u.z;
  ...
  if (action == GLFW_PRESS) {
    ...
    if (app->currentMode == EditorMode::EDIT && app->mapEditor) {
      ...
      if (mods & GLFW_MOD_CONTROL) {
        if (key == GLFW_KEY_N)
          app->mapEditor->NewMap();
        if (key == GLFW_KEY_S)
          app->mapEditor->SaveMap();
        if (key == GLFW_KEY_Z)
          app->mapEditor->Undo();
        if (key == GLFW_KEY_Y)
          app->mapEditor->Redo();
        if (key == GLFW_KEY_D)
          app->mapEditor->DuplicateSelected();
        if (key == GLFW_KEY_A)
          app->mapEditor->SelectAll();
      } else {
        if (key == GLFW_KEY_1)
          app->mapEditor->SetTool(PCD::EditorTool::SELECT);
        if (key == GLFW_KEY_2)
          app->mapEditor->SetTool(PCD::EditorTool::MOVE);
        if (key == GLFW_KEY_3)
          app->mapEditor->SetTool(PCD::EditorTool::ROTATE);
        if (key == GLFW_KEY_4)
          app->mapEditor->SetTool(PCD::EditorTool::SCALE);
        if (key == GLFW_KEY_5)
          app->mapEditor->SetTool(PCD::EditorTool::CREATE_BOX);

        if (key == GLFW_KEY_DELETE)
          app->mapEditor->DeleteSelected();
        if (key == GLFW_KEY_ESCAPE)
          app->mapEditor->DeselectAll();

        if (key == GLFW_KEY_B)
          app->mapEditor->SetTool(PCD::EditorTool::CREATE_BOX);
        if (key == GLFW_KEY_C)
          app->mapEditor->SetTool(PCD::EditorTool::CREATE_CYLINDER);
      }
    }
  }
}

void EditorApp::MouseButtonCallback(GLFWwindow *window, int button, int action,
                                    int mods) {
  EditorApp *app = GetInstance(window);
  if (!app)
    return;
  ...
  if (button == GLFW_MOUSE_BUTTON_LEFT) {
    if (action == GLFW_PRESS) {
      if (app->isAltPressed) {
        ...
        // Use ray-based clicking
        app->mapEditor->OnMouseClickWithRay(mx, my, width, height, view, proj,
                                            shift);
      }
...
void EditorApp::ScrollCallback(GLFWwindow *window, double xoffset,
                               double yoffset) {
  EditorApp *app = GetInstance(window);
  if (!app)
    return;
...
void EditorApp::MouseCallback(GLFWwindow *window, double xpos, double ypos) {
  ...
  if (app->currentMode == EditorMode::EDIT) {
    ...
    if (app->mapEditor && app->isLeftDragging &&
      ...
      float worldDY = dy * moveScale; // Direct vertical movement
      ...
      // Use actual world coordinates for better axis-constrained movement
